...
class LLMArchetypeBuilder:
    ...
    def __init__(
        ...
        if fallback_to_mock:
            try:
                ...
                logger.warning(f"Could not import mock builder: {e}")
    ...
    async def _generate_initial_archetypes(
        ...
        prompt = self.prompts.ARCHETYPE_GENERATION.format(
            ...
            positioning=getattr(brand_context, 'brand_positioning', 'Not specified'),  # <-- CHANGE THIS
    ...
    def _merge_behavioral_refinements(
        ...
        for archetype in original_archetypes:
            ...
            refinement = refinement_lookup.get(arch_id, {})  # <-- ADD THIS LINE
    ...
    def _fallback_to_mock(
        ...
        if hasattr(self, 'mock_builder') and self.mock_builder:
            ...
            logger.warning("Using emergency fallback - mock builder not available")
            category = category_intelligence.get("category", "product")
